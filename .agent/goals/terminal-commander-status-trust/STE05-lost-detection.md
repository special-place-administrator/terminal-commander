---
goal_id: STE05
title: Lost Detection
chain_id: terminal-commander-status-trust
phase: Wave 3 - Diagnosis
status: "Pending"
depends_on: [STE01]
...
source_refs:
  ...
  - "specs/004-status-evidence-trust/tasks.md (T033-T037)"
...
Distinguish a job the engine recorded starting but never recorded finishing from an identifier it never knew, using the per-lane start records that already exist.
...
objective:
- Complete tasks T033-T037 in `specs/004-status-evidence-trust/tasks.md`.
...
invariants:
- Predicate on command_start / pty_command_start / file_watch_start - NOT job_start, which is dead code (research R2).
- Filter decision='allow'.
- An inconclusive lookup degrades to unknown, never to a terminal outcome.
...
Run STE05 only on branch `004-status-evidence-trust`. Route every build and test through Terminal
