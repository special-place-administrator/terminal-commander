---
goal_id: STE01
title: Outcome Evidence Foundation
...
source_refs:
  ...
  - "specs/004-status-evidence-trust/tasks.md (T001-T009)"
...
# STE01 - Outcome Evidence Foundation
...
Add the V0008 additive migration, the receipt evidence read/write, the subject-filtered audit read, and the wire vocabulary (`OutcomeTrust`, `outcome_trust`, `JobLost`). Nothing else may begin until this lands.
...
objective:
- Complete tasks T001-T009 in `specs/004-status-evidence-trust/tasks.md`.
...
- `crates/daemon/src/store_actor.rs`
- `crates/daemon/src/ipc/handlers/common.rs`
...
invariants:
- Do not change any counter field type - optionality is rejected (research R4).
- Do not add a JobState variant - decision D1.
- Do not backfill or rewrite existing receipt rows.
...
Run STE01 only on branch `004-status-evidence-trust`. Route every build and test through Terminal
